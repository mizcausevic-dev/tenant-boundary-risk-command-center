// Structure + data-integrity tests for Tenant Boundary Risk Command Center
...
test("HTML5 doctype + lang + title", () => {
  ...
  assert.match(html, /<title>Tenant\s+Boundary\s+Risk\s+Command\s+Center/);
});
...
test("4 stat tiles match synthetic data", () => {
  ...
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">30<\/span><span class="stat-label">Total\ tenants/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">8<\/span><span class="stat-label">T1\ Enterprise/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">1<\/span><span class="stat-label">RLS\-missing\ tenants/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">9<\/span><span class="stat-label">Open\ audit\ findings/);
});
...
test("First anomaly card content present (data-integrity proxy)", () => {
  assert.ok(html.includes("Mariner Coffee"), "First anomaly substring missing from rendered HTML");
});
...
test("First audit event kind present (data-integrity proxy)", () => {
  assert.ok(html.includes("tenant.boundary.cross-access-detected"), "First audit event kind missing from rendered HTML");
});
...
test("CNAME matches expected subdomain", () => {
  ...
  assert.equal(cname, "mt.kineticgain.com");
});
